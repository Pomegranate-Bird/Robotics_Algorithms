...
def circle_figure(t, size = 5): # I just want to throw trajectories into the controller so I can make different ones and throw it into the same p-controller
    ...
    """
    N = size # Representing the radius of the circle 
    # Position 
    x_d = np.cos(t) * N 
    y_d = np.sin(t) * N
...
def pid_controller(robot_id, t, speed_gain = 1, steering_gain = 5, integral_gain = .25):
    ...
    k_v = speed_gain  # Controller gain, to adjust changes in velocity error
    k_s = steering_gain # Controller gaim, to adjust the changes in the steering error

    past_error = trapezoid(trajectory_error) # Integral of the trajectory_error

    past_error = np.linalg.norm(past_error)

    velocity = (k_v * distance_error) * (past_error * integral_gain)
    
    steering_angle = (k_s * heading_error) * (past_error * integral_gain)
